...
public class Medium {
    ...
    public int minEatingSpeed(int[] piles, int h) {
        ...
        // High is the max pile size because eating faster than the largest pile doesn't save more time
        int high = Arrays.stream(piles).max().getAsInt();
        while (low <= high) {
            int mid = (low + high) / 2;

            // If total hours at 'mid' speed fits within 'h', try a slower speed to find the minimum
    ...
    private int sumOfDivisor(int[] arr, int d) {
        ...
        for (int i = 0; i < arr.length; i++) {
            sum += Math.ceil((double) arr[i] / (double) d);     // Ceiling division: Any remainder increments the result by 1 for that element
        }
        return sum;
    }
    ...
    public int smallestDivisor(int[] nums, int threshold) {
        ...
        int high = Arrays.stream(nums).max().getAsInt();    // Largest useful divisor is the max element; any larger divisor results in the same sum (1 per element)
        int ans = -1;
        while (low <= high) {
            int mid = (low + high) / 2;

            // If the current sum is within the threshold, try a smaller divisor to minimize the answer
    ...
    public int kthMissing(int[] arr, int k) {
        ...
        while (low <= high) {
            ...
            // Number of missing integers before index 'mid' is: (actual value) - (expected value if none were missing)
            int missing = arr[mid] - (mid + 1);

            // If the number of missing values is less than k, the kth missing must be further to the right
            if (missing < k)
                low = mid + 1;
            else
                high = mid - 1;
    ...
    public boolean canWePlaceCows(int[] stalls, int minDist, int cows) {
        ...
        for (int i = 1; i < stalls.length; i++) {
            ...
            // Only place the next cow if the distance from the last placed cow is at least 'minDist'
    ...
    public int aggressiveCows(int[] stalls, int k) {
        ...
        // Range: Minimum possible distance is 1, Maximum is the distance between first and last stall
        int low = 1, high = stalls[n - 1] - stalls[0];
        int ans = -1;
        while (low <= high) {
            int mid = (low + high) / 2;

            // If it's possible to place cows with 'mid' distance, try to increase the distance for a better result
    ...
    public static void main(String[] args) {
        ...
        int[] nums = { 1, 2, 4, 8, 9 };
        System.out.println(medium.aggressiveCows(nums, 3));
    }
}